...
import { actionCancelSubscription } from './cancelSubscription';

const PERIOD_END_ISO = '2026-09-10T00:26:40.000Z';
const CANCEL_AT_ISO = '2026-09-10T00:26:40.000Z';

describe('actionCancelSubscription', () => {
  beforeEach(() => {
  ...
  it('IT-008 sends the cancellation date and the period end', async () => {
    ...
    const [result, error] = await actionCancelSubscription();

    expect(error).toBeNull();
    expect(result).toEqual({ cancelAt: CANCEL_AT_ISO });
    ...
    expect(JSON.parse(patchCall?.[1].body as string)).toEqual({
      ...
      subscriptionCancelAt: CANCEL_AT_ISO,
